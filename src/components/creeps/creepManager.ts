...
function _harvestSource(source: Source) {
  ...
  if (!silo) {
    ...
    silo = _.find(squares, (sq) => sq.type == 'structure' && (sq.structure as Structure).structureType == STRUCTURE_CONTAINER).structure as StructureContainer;
    if (!silo) {
      ...
      silo = source.room.find<ConstructionSite>(FIND_CONSTRUCTION_SITES, {filter: (s :ConstructionSite) => s.pos.x == squares[0].x && s.pos.y == squares[0].y})[0];
    } else {
  ...
  if (!harvester) {
    ...
    // now make it
    let spawn = source.pos.findClosestByPath<StructureSpawn>(FIND_MY_SPAWNS, {filter: (s: StructureSpawn) => !s.spawning});
...
function _buildJobs(room: Room) {
  ...
  let constSites = room.find<ConstructionSite>(FIND_MY_CONSTRUCTION_SITES);
  _.each(constSites, (s) => jobs.push(new job.Job(s, job.JobAction.Build)));

  // things that need energy
  let drained = room.find<Structure>(FIND_MY_STRUCTURES, {filter: (s: Structure) => {
...
function _assignJobs(jobs: job.Job[]) {
  _.each(jobs, (j: job.Job) => {
    ...
    let workersOnJob = _.filter(workers, (w) => j.equals(w.job));
 ...
 * @returns
 */
function _spawnCreep(spawn: Spawn, bodyParts: string[], role: string, properties?: { [key: string]: any }) :[string, number] {
  ...
  properties.room = spawn.room.name;
