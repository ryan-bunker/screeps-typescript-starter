...
export class Base {
  ...
  protected findClosestSilo(): StructureContainer {
    let silo = this._creep.pos.findClosestByPath<StructureContainer>(FIND_STRUCTURES, {filter: (s: Structure) =>
      s.structureType == STRUCTURE_CONTAINER && (s as StructureContainer).store > 0});
    return silo;
  }

  protected findClosestSource(): Source {
    let source = this._creep.pos.findClosestByPath<Source>(FIND_SOURCES);
    return source;
  }
